...
class AbstractViewModal extends AbstractView
	...
	close : =>
		...
		@B().router.navigateTo @B().appView.wrapper.backgroundView.route
	...
	animateInDone : (cb) =>
		...
		@B().appView.modalPlayBtn.show()

		null

	animateOut : (cb) =>

		@B().appView.modalPlayBtn.hide()
